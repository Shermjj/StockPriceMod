...
test_that("Correct number of principal components are returned", {
  result <- pca(data_matrix, cumulative_var = 0.9)
...
test_that("Handles different variance thresholds correctly", {
  for (threshold in seq(0.7, 0.95, by = 0.05)) {
    result <- pca(data_matrix, cumulative_var = threshold)
    cumulative_variance <- sum(result$variance_explained)
    expect_true(cumulative_variance >= threshold)
  }
})

# Test 3: Check for appropriate error or warning messages
test_that("Errors and warnings are handled correctly", {
  # Example: Testing for a negative variance threshold
  expect_error(pca(data_matrix, cumulative_var = -0.1))
