...
# Experiments
We utilise our developed package, "StockPriceMod" which is available on github [here](https://github.com/Shermjj/StockPriceMod) to perform our portfolio optimisation over the year 2020. This package can be install with the command `devtools::install_github('Shermjj/StockPriceMod')`. 
...
var_list <- list()
weights_list <- list()
mu_list[[1]] <- pred_returns
var_list[[1]] <- time_series_df[[2]]
weights_list[[1]] = QP_result$solution

for(idx in seq(1,24)){
  start_day <- idx * 10
  end_day <- start_day + 10 # We use a 10 day rolling window
  ...
  pred_returns <- time_series_df$mu - tail(y,1) #z_T+1
  mu_list[[idx+1]] <- pred_returns
  var_list[[idx+1]] <- time_series_df[[2]]
  covar_mat <- diag(time_series_df[[2]])
  weights_list[[idx+1]] <- port_opt(covar_mat, pred_returns, num_components)
}
```
